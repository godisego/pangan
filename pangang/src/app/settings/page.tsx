...
import { chatApi, notifyApi } from '@/lib/api';
import { FALLBACK_AI_PROVIDERS, getProviderById } from '@/lib/aiProviders';
import { defaultSettings, getPersistedSettingsSignature, loadUserSettings, saveUserSettings, type UserSettings } from '@/lib/localSettings';
import type {
...
function StatusCard({
  ...
  return (
    <div className="module-node">
      ...
      {detail ? <div className="module-node__copy">{detail}</div> : null}
    </div>
  );
}
...
export default function SettingsPage() {
  ...
  const [savedSignature, setSavedSignature] = useState(getPersistedSettingsSignature(defaultSettings));
  const [remoteSyncSignature, setRemoteSyncSignature] = useState('');
  ...
  useEffect(() => {
    ...
    const persistedSignature = getPersistedSettingsSignature(normalized);
    setSettings(normalized);
    setSavedSignature(persistedSignature);
    setRemoteSyncSignature('');
    ...
    void chatApi.getProviders()
      .then((catalog) => {
        ...
        setSavedSignature((prevSignature) => {
          ...
          return getPersistedSettingsSignature(normalizeAiSettings(parsed, catalog.providers));
        });
      })
      .catch(() => {
        // 保持本地兜底清单即可，避免设置页空白
  ...
  const summarizeNotifyResult = (json: NotificationBroadcastResponse, fallbackPrefix: string) => {
    ...
    const details = Object.entries(results).map(([channel, result]) => {
      const typedResult = result as NotificationChannelResult;
      const label =
        channel === 'telegram'
          ? 'Telegram'
          : channel === 'feishu'
      ...
      if (typedResult.message) return `${label}：${typedResult.message}`;
      if (typedResult.ok === true || typedResult.errcode === 0 || typedResult.StatusCode === 0 || typedResult.code === 0) {
        return `${label}：发送成功`;
      }
  ...
  const persistedSignature = useMemo(() => getPersistedSettingsSignature(settings), [settings]);
  ...
  const status = useMemo(() => {
    ...
    const localSaved = persistedSignature === savedSignature;
    const saveState = !localSaved
      ? '未保存'
      : !notifyConfigWriteEnabled
        ? '仅本地保存'
        : persistedSignature === remoteSyncSignature
          ? '已同步'
          : '待同步';
    ...
    return {
      ...
      saveState,
      localSaved,
      remoteSynced: notifyConfigWriteEnabled && persistedSignature === remoteSyncSignature,
    };
  }, [hasLocalAiConfig, notifyConfigWriteEnabled, persistedSignature, remoteSyncSignature, savedSignature, settings, sharedAi?.enabled]);
  ...
  const currentAiLabel = hasLocalAiConfig
  ...
  useEffect(() => {
    if (!notifyConfigWriteEnabled) {
      setRemoteSyncSignature(savedSignature);
    }
  }, [notifyConfigWriteEnabled, savedSignature]);

  const handleSave = async () => {
    const nextPersistedSignature = getPersistedSettingsSignature(settings);
    setPendingAction('save');
    saveUserSettings(settings);
    setSavedSignature(nextPersistedSignature);

    if (!notifyConfigWriteEnabled) {
      setRemoteSyncSignature(nextPersistedSignature);
      setMsg({ type: 'success', text: '本地偏好已保存。当前部署不支持远端通知配置写入。' });
      setPendingAction('');
      return;
    }

    try {
      const json = await notifyApi.saveConfig({
        channels: {
  ...
  const handleTest = async () => {
    if (!notifyTestEnabled) {
      setMsg({ type: 'info', text: '当前部署不支持远程通知测试。通知能力由站点拥有者在服务端统一维护。' });
      return;
    }
    ...
    try {
      ...
      setMsg({ type: 'info', text: '正在测试已配置通道...' });
      const json = await notifyApi.test({
        title: '盘感配置中心测试',
  ...
  const handleDailyTest = async () => {
    if (!notifyTestEnabled) {
      setMsg({ type: 'info', text: '当前部署不支持手动触发日报测试，请由站点拥有者在服务端触发。' });
      return;
    }
    const hasAnyChannel = Boolean(
      settings.notifications.feishuWebhook ||
    ...
    try {
      ...
      const json = await notifyApi.sendDailyReport({
        config: {
          feishu_webhook: settings.notifications.feishuWebhook,
          wecom_webhook: settings.notifications.wecomWebhook,
          telegram_bot_token: settings.notifications.telegramBotToken,
  ...
  return (
    <AppShell
      title="设置"
      subtitle="先看当前能力，再按需配置 AI、通知和个人偏好。"
    ...
    >
      ...
      >
        <div className="module-kpi-grid">
          <StatusCard
            label="AI"
            value={status.aiReady ? '可用' : '未就绪'}
            detail={sharedAi?.enabled && !hasLocalAiConfig ? '当前可直接使用平台共享 AI' : hasLocalAiConfig ? '当前优先使用你的本地配置' : '还没有可用模型'}
          />
          <StatusCard
            label="当前模型"
            value={currentAiLabel}
            detail={sharedAi?.enabled && !hasLocalAiConfig ? '未填写个人 Key 时会直接走平台共享模型' : '可在下方改成你自己的模型配置'}
          />
          <StatusCard
            label="通知"
            value={status.pushReady ? `${status.channelCount} 个通道` : '未配置'}
            detail={notifyConfigWriteEnabled ? '通知配置支持写入服务端' : '当前只保留本地偏好，不写入远端'}
          />
          <StatusCard
            label="通知测试"
            value={notifyTestEnabled ? '可测试' : '已关闭'}
            detail={notifyTestEnabled ? '可直接验证已配置通道' : '当前部署不开放远程测试'}
          />
        </div>
      ...
      <ModuleShell
        title="AI 配置"
        summary="共享 AI 可用时，不填个人 Key 也能直接用；只有想覆盖成自己的模型时才需要填写。"
        badge={status.aiReady ? '已就绪' : '待配置'}
        variant="settings"
        motion="drift"
        compact
        actions={(
          <button onClick={() => void handleAiTest()} className="btn btn-secondary px-4 py-2 text-sm">
            {pendingAction === 'ai-test' ? '验证中...' : sharedAi?.enabled && !hasLocalAiConfig ? '测试当前 AI' : '测试 AI'}
          </button>
        )}
      >
        {sharedAi?.enabled ? (
          <div className="rounded-[18px] border border-[rgba(105,231,176,0.22)] bg-[rgba(105,231,176,0.08)] px-4 py-3 text-sm leading-6 text-[var(--text-secondary)]">
            平台共享 AI 已可用：{sharedProvider?.label || sharedAi.provider_label || sharedAi.provider} · {sharedAi.model || '默认模型'}。
            {!hasLocalAiConfig ? ' 你现在不填个人 Key 也能直接在首页和对话页使用 AI。' : ' 当前仍会优先使用你的个人配置。'}
          </div>
        ) : null}
        ...
        <Field label={selectedProvider.api_key_label} hint={selectedProvider.description}>
          <input
            type="password"
            className={inputClassName}
      ...
      <ModuleShell
        title="通知配置"
        summary={notifyConfigWriteEnabled ? '飞书、企微、Telegram 三选一即可。' : '当前部署不支持远端写入，这里主要用于查看和保留本地配置。'}
      ...
      >
        ...
        <div className="module-node">
          ...
          <div className="module-node__copy">先给机器人发 /start，再找 @userinfobot 或 @RawDataBot 获取数字 Chat ID。</div>
        </div>
      </ModuleShell>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <ModuleShell
          title="个人偏好"
          summary="这些设置只影响你的使用习惯，不改变系统真实能力。"
          badge={settings.preferences.enablePush ? settings.preferences.pushTime : '不定时推送'}
          variant="settings"
          motion="track"
          compact
        >
        ...
        <ModuleShell
          title="保存与测试"
          summary={notifyConfigWriteEnabled ? '保存时会同时写入本地和后端通知配置。' : '当前只保存本地偏好。'}
          ...
          <div className="grid gap-3">
            ...
            {notifyTestEnabled ? (
              ...
              <div className="rounded-[18px] border border-[var(--border-color)] bg-[rgba(255,255,255,0.02)] px-4 py-3 text-sm leading-6 text-[var(--text-secondary)]">
                当前部署未开放通知测试，因此这里只保留保存能力，不再把测试动作放成主按钮。
              </div>
            )}

            <div className="rounded-[18px] border border-[var(--border-color)] bg-[rgba(255,255,255,0.02)] px-4 py-3 text-sm leading-6 text-[var(--text-secondary)]">
              {notifyConfigWriteEnabled
                ? '如果后端同步失败，本地偏好仍会保留；你可以先继续使用，再回来看同步状态。'
                : '这套共享部署主要开放看盘和 AI 能力，通知配置建议由站点拥有者统一维护。'}
            </div>
          </div>
        </ModuleShell>
      </div>
